...
async def log_bot_action_to_debug(bot: Bot, target_chat_id, text_of_message, is_cmd_action=False, reply_msg_id=None, effective_user_id=None):
    ...
    target_entity_type = "USER" if chat_id_for_log > 0 and user_id_for_log == chat_id_for_log else "CHAT"
    
    log_message_header = f"🔹 BOT TO {target_entity_type} {user_id_for_log if target_entity_type == 'USER' else chat_id_for_log}: [{action_description}]"
    if reply_msg_id:
        log_message_header += f" (Reply to {reply_msg_id})"
    
    if target_entity_type == "CHAT" and user_id_for_log != 123456789:
         log_message_header += f" (User ID: {user_id_for_log})"
    if target_entity_type == "USER" and chat_id_for_log != user_id_for_log and chat_id_for_log < 0:
        log_message_header += f" (Chat ID: {chat_id_for_log})"
